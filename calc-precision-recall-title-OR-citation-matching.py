...
def main(args):
    ...
    csvwriter.writerow(['Measure', 'Threshold', 'Title Jaccard', 'Citation Title Jaccard', 'non-NULL', 'non-NULL ratio', 'Precision', 'Recall', 'F1', 'True Pos', 'False Pos', 'False Neg'])
            ...
            for tjc_threshold in title_jaccards:
                for not_null_threshold in not_null_thresholds:
                    for not_null_ratio_threshold in not_null_ratio_thresholds:
                        print("title jaccard = %.1f, not NULL threshold = %d, not NULL ratio threshold = %.1f" % (tjc_threshold, not_null_threshold, not_null_ratio_threshold))

                        for threshold in thresholds:
                            true_pos, false_pos, false_neg = calculate(args.infiles, tjc_threshold, not_null_threshold, not_null_ratio_threshold, field, threshold)
                            ...
                            csvwriter.writerow([measure_name, threshold, tjc_threshold, cjc, not_null_threshold, not_null_ratio_threshold, precision, recall, f1, true_pos, false_pos, false_neg])
            print()
